...
# These libraries are bundled in /usr/share/rimtidy/ and should not be system requirements
...
Release:        1%{?dist}
Summary:        RimTidy - Mod manager for RimWorld
...
%description
RimTidy is an open source mod manager for the video game RimWorld.
...
# Install the entire Nuitka build output to /usr/share/rimtidy
cp -pr build/app.dist/* %{buildroot}%{_datadir}/%{name}/

# Create wrapper script in /usr/bin
cat > %{buildroot}%{_bindir}/%{name} << 'EOF'
#!/bin/bash
exec %{_datadir}/%{name}/RimTidy "$@"
EOF
...
- Initial RPM packaging for COPR (renamed to RimTidy)
