#!/usr/bin/env python3
"""
This script is used to set up the environment and build the RimTidy application.
...
def get_latest_todds_release() -> None:
    ...
    if _SYSTEM == "Darwin":
        ...
        print("Skipping todds download. The resultant RimTidy build will not include todds!")
        return
...
def freeze_application() -> None:
    """Build the RimTidy application using Nuitka."""
...
def make_args() -> argparse.ArgumentParser:
    """
    Create and configure the argument parser for the RimTidy application setup script.

    Returns:
        argparse.ArgumentParser: The configured argument parser.
    """
    description = """This script is used to set up the environment and build the RimTidy application.
    It installs the required dependencies, initializes and updates submodules, and compiles the SteamworksPy library.
    The script supports different operating systems and architectures."""
    ...
    parser.add_argument(
        "--skip-submodules",
        action="store_true",
        help="skip installing RimTidy submodules using git",
    )
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="skip building RimTidy with Nuitka",
    )
...
def main() -> None:
    ...
    if not args.skip_submodules:
        print("Getting RimTidy submodules...")
    ...
    if not args.skip_build:
        ...
        print("Building RimTidy application with Nuitka...")
        freeze_application()
