    ...
    try:
        ...
        dialogue.show_fatal_error(
            ...
            text=translate(
                ...
                "RimTidy failed to copy the text to your clipboard. Please copy it manually.",
            ),
            details=str(e),
        )
...
def rmtree(path: str | Path, **kwargs: Any) -> bool:
    ...
    if not path.exists():
        ...
        dialogue.show_warning(
            ...
            text=translate("rmtree", "RimTidy tried to remove a directory that does not exist."),
    ...
    if not path.is_dir():
        ...
        dialogue.show_warning(
            title=translate("rmtree", "Failed to remove directory"),
            text=translate("rmtree", "RimTidy tried to remove a directory that is not a directory."),
    ...
    if not game_install_path:
        ...
        dialogue.show_warning(
            ...
            information=(
                translate(
                    "launch_game_process",
                    "RimTidy could not start RimWorld as the game folder is empty or invalid: [{game_install_path}] "
    ...
    if not executable_path:
        ...
        dialogue.show_warning(
            ...
            information=(
                translate(
                    "launch_game_process",
                    "RimTidy could not validate the RimWorld executable in the specified folder: {game_install_path}. Please check that this directory is correct and contains a valid RimWorld game executable.",
                ).format(game_install_path=game_install_path)
            ),
        )
        return
...
def upload_data_to_0x0_st(path: str) -> tuple[bool, str]:
    ...
    try:
        with open(path, "rb") as f:
            headers = {"User-Agent": f"RimTidy/{AppInfo().app_version}"}
