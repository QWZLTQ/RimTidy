...
class AppInfo:
    ...
    def __init__(self) -> None:
        ...
        # Migrate data from old "RimSort" directory if new directory is empty/missing
        old_platform_dirs = PlatformDirs(appname="RimSort", appauthor=False)
        old_storage = Path(old_platform_dirs.user_data_dir)
        if old_storage.exists() and not self._app_storage_folder.exists():
            import shutil
            shutil.copytree(str(old_storage), str(self._app_storage_folder))
