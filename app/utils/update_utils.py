...
UNKNOWN_VERSION_TEXT = "Could not find version Information of RimTidy, This may indicate that the application is not an offical release and may be a custom build."
...
ERR_UPDATE_ERROR_TITLE = "RimTidy Update Error"
ERR_NO_VALID_RELEASE_TITLE = "RimTidy Update Error"
ERR_NO_VALID_RELEASE_TEXT = "Failed to find valid RimTidy release for {system_info}"
ERR_API_CONNECTION_TITLE = "RimTidy Update Error"
...
class UpdateManager(QObject):
    ...
    def do_check_for_update(self) -> None:
        """
        Check for RimTidy updates and handle the update process.
    ...
    def _fetch_and_compare_versions(self) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Current RimTidy version: {current_version}")
        ...
        logger.info(f"Latest RimTidy version: {latest_version}")
    ...
    def _prompt_user_for_update(self, latest_tag_name: str, current_version: str) -> bool:
        ...
        """
        answer = dialogue.show_dialogue_conditional(
            title=self.tr("RimTidy update found"),
            text=self.tr("An update to RimTidy has been released: {latest_tag_name}").format(
                latest_tag_name=latest_tag_name
            ),
            information=self.tr("You are running RimTidy {current_version}\nDo you want to update now?").format(
    ...
    def _perform_update(self, download_url: str, tag_name: str, is_msi: bool) -> None:
        ...
        """
        try:
            logger.debug(f"Downloading & extracting RimTidy release from: {download_url}")

            # Download with progress widget
            self._download_cancelled = False
            self._progress_widget = TaskProgressWindow(
                title="RimTidy Update",
                show_message=True,
                show_percent=True,
            )
            self._progress_widget.update_progress(
                0,
                self.tr("Downloading RimTidy {tag_name} release...").format(tag_name=tag_name),
            )
    ...
    def _create_temp_dir(self) -> Path:
        ...
        """
        temp_base = Path(gettempdir()) / f"RimTidy_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def _extract_update(self, is_msi: bool = False) -> Path:
        ...
        try:
            ...
            if is_msi:
                ...
                msi_path = temp_base / "RimTidy_Update.msi"
    ...
    def _validate_executable_presence(self, extract_path: Path, children: List[Path]) -> None:
        ...
        """
        expected_executable = "RimTidy.exe" if self._system == "Windows" else "RimTidy"
        ...
        if self._system == "Darwin":
            ...
            if not executable_found:
                logger.error("RimTidy.app bundle not found")
                raise UpdateExtractionError("Expected RimTidy.app bundle with executable not found after normalization")
        else:
    ...
    def _launch_windows_update_script(
        ...
        if needs_elevation:
            ...
            start_cmd = f'start "RimTidy Update" /D "{cwd}" {cmd_str}'
    ...
    def _launch_msi_installer(self, msi_path: Path, log_path: Path, needs_elevation: bool) -> None:
        ...
        The MSI is configured with CustomActions to automatically launch RimTidy.exe
        ...
        try:
            ...
            logger.info("MSI launched, exiting RimTidy to allow file replacement")
    ...
    def _prepare_update_log(self, system: str) -> Path:
        ...
        try:
            with open(log_path, "a", encoding="utf-8", errors="ignore") as lf:
                lf.write(
                    f"\n===== RimTidy updater launched: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ({system}) =====\n"
                )
        except Exception:
    ...
    def _create_backup(self) -> None:
        """
        Create a compressed backup of the current RimTidy installation as a ZIP file.
        ...
        backup_filename = f"RimTidy_Backup_{timestamp}.zip"
        backup_path = app_backup_folder / backup_filename

        logger.info("Creating backup of RimTidy installation")
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            ...
            backup_files = list(app_backup_folder.glob("RimTidy_Backup_*.zip"))
