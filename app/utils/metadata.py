...
class MetadataManager(QObject):
    ...
    def __init__(self, settings_controller: SettingsController) -> None:
        if not hasattr(self, "initialized"):
            ...
            # Initialize external metadata loader
            self.external_loader = ExternalMetadataLoader(self)
    ...
    def __read_game_version(self) -> None:
        ...
        if version_file_path.exists():
            ...
            self.show_warning_signal.emit(
                ...
                self.tr(
                    "RimTidy is unable to get the game version at the expected path: [{version_file_path}]."
    ...
    def __parse_mod_metadata(
        ...
        if (invalid_about_file_path_found and not scenario_rsc_found) or data_malformed:
            ...
            metadata[uuid] = {
                ...
                "authors": "Not found",
                "description": (
                    "This mod is considered invalid by RimTidy (and the RimWorld game)."
...
class SteamDatabaseBuilder(QThread):
    ...
    def run(self) -> None:
        ...
        self.db_builder_message_output_signal.emit(
            f"\nInitiating RimTidy Steam Database Builder with mode : {self.mode}\n"
        )
...
def check_if_pfids_blacklisted(publishedfileids: list[str], steamdb: dict[str, Any]) -> list[str]:
    ...
    if not steamdb:
        show_warning(
            ...
            text="Unable to check for blacklisted mods. Please configure a SteamDB for RimTidy to use in Settings.",
        )
