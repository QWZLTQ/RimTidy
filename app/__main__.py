...
# nuitka-project: --output-filename=RimTidy
...
# nuitka-project: --include-data-dir={MAIN_DIRECTORY}/qml=app/qml
...
def handle_exception(
    ...
    if issubclass(exc_type, KeyboardInterrupt):
        ...
        show_fatal_error(
            title="RimTidy crashed",
            text="The RimTidy application crashed! Sorry for the inconvenience!",
...
if __name__ == "__main__":
    # If RimTidy is running from a --onefile Nuitka build, there are some nuances to consider:
    ...
    # Default UI is QML; use --use-widgets to launch legacy Qt Widgets UI
    if "--use-widgets" in sys.argv:
        sys.argv.remove("--use-widgets")
        pass  # fall through to Qt Widgets launcher below
    elif "--use-qml" in sys.argv:
        sys.argv.remove("--use-qml")  # backwards compat
        from app.bridge.qml_app import launch_qml_app
        sys.exit(launch_qml_app())
    else:
        from app.bridge.qml_app import launch_qml_app
        sys.exit(launch_qml_app())
    ...
    logger.info(f"Initializing RimTidy application: {AppInfo().app_version}")
    main_thread()
