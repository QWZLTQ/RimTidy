    ...
    def _on_search_error(self, error_msg: str) -> None:
        ...
        if "regex" in error_msg.lower():
            ...
            show_warning(
                ...
                text=self.tr("RimTidy doesn't have permission to access some files."),
                information=self.tr(
                    "{error_msg}\n\nTry running RimTidy with administrator privileges or check folder permissions."
                ).format(error_msg=error_msg),
            )
        elif "memory" in error_msg.lower():
            show_warning(
                title=self.tr("Memory Error"),
                text=self.tr("RimTidy ran out of memory while searching."),
                information=self.tr(
