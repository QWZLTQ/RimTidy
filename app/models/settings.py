...
class Settings(QObject):
    ...
    def handle_corrupted_settings(self) -> None:
        ...
        if (AppInfo().settings_backups_folder / "settings.json.backup").exists():
            msg += (
                f"RimTidy found a backup at {AppInfo().settings_backups_folder / 'settings.json.backup'}. "
            ...
            msg += (
                f"RimTidy found an old backup at {AppInfo().settings_backups_folder / 'settings.json.backup.old'}. "
                "Do you want to attempt to recover your settings from this backup?"
            )
            use_old_backup = True
        else:
            msg += "No backup file was found, RimTidy will reset your settings to defaults."

        dlg = BinaryChoiceDialog(
            title=self.tr("Settings Load Error"),
            text=self.tr(msg),
            information=self.tr(
                f"If you proceed, a backup of the corrupted file will be saved to {AppInfo().settings_backups_folder / 'settings.json.corrupted'}."
            ),
            positive_text=self.tr("Proceed"),
            negative_text=self.tr("Exit RimTidy"),
        )
        if dlg.exec_is_positive():
            try:
                ...
                InformationBox(
                    ...
                    text=self.tr(
                        "RimTidy failed to recover your settings from the backup. "
