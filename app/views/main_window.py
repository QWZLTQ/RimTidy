...
from app.views.custom_title_bar import CustomTitleBar
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, settings_controller: SettingsController, debug_mode: bool = False) -> None:
        ...
        app_layout.addWidget(self.custom_title_bar)

        # Create a standalone menu bar (not native QMainWindow menuBar)
        self._standalone_menu_bar = QMenuBar(self)
        app_layout.addWidget(self._standalone_menu_bar)
        ...
        # Size grip for resizing frameless window (bottom-right corner)
        ...
        app_layout.addLayout(grip_layout)
        ...
        self.menu_bar = MenuBar(menu_bar=self._standalone_menu_bar, settings_controller=self.settings_controller)
    ...
    def _apply_rounded_corners(self) -> None:
        """Apply rounded corners and shadow on Win11. No-op on Win10/other."""
        ...
        try:
            ...
            hwnd = int(self.winId())
            # DWMWA_WINDOW_CORNER_PREFERENCE = 33, only works on Win11 22000+
            ...
            logger.debug("Applied Win11 DWM rounded corners and shadow")
        except Exception as e:
            logger.debug(f"DWM effects not available (Win10 or older): {e}")

    def _launch_main_window(self) -> None:
        """Apply main window launch state from settings, default to centered normal size."""
        ...
        # For frameless window, default to centered normal size instead of maximized
    ...
    def __check_steam_integration(self) -> None:
        """Ask the user if they would like to enable Steam Client Integration for the active instance if it is the first time they are setting up RimTidy."""
        ...
        if instance.initial_setup and not instance.steam_client_integration:
            diag = BinaryChoiceDialog(
                ...
                information=self.tr("""This will allow you to use RimTidy features that require the Steam Client. This includes, among other things, unsubscribing from workshop mods and opening workshop links via the Steam Client.
                <br><br>
        ...
        answer = show_dialogue_conditional(
            ...
            information=(
                self.tr(
                    ...
                    + "RimTidy will copy all Workshop mods to the new instance's local mods folder, converting them to SteamCMD mods that you can manage inside the new instance. The Workshop folder will be ignored for this instance to prevent duplicate mods.\n\n"
    ...
    def __set_window_title(self, instance: str) -> None:
        ...
        """
        title = f"RimTidy {AppInfo().app_version} | {instance} Instance"
        self.setWindowTitle(title)
        if hasattr(self, "custom_title_bar"):
            self.custom_title_bar.set_title("RimTidy")
