...
class MainContent(QObject):
    ...
    def check_if_essential_paths_are_set(self, prompt: bool = True) -> bool:
        ...
        if (
            ...
            answer = dialogue.show_dialogue_conditional(
                ...
                information=(
                    self.tr(
                        "RimTidy requires, at the minimum, for the game install folder and the "
    ...
    def _do_check_for_update(self) -> None:
        """
        Check for RimTidy updates using UpdateManager.
        """
            ...
            def notify_user() -> None:
                ...
                dialogue.show_information(
                    ...
                    text=self.tr(
                        ...
                        "If RimTidy is still not able to download some mods, "
    ...
    def _build_rentry_report(
        ...
        active_mods_rentry_report = (
            ...
            + f"\nCreated with RimTidy {AppInfo().app_version}"
    ...
    def _do_open_rimsort_logs_directory(self) -> None:
        ...
        logger.info(f"Opening RimTidy logs directory: {logs_directory}")
    ...
    def _create_todds_runner(self, is_pre_launch: bool) -> RunnerPanel:
        ...
        base_title = "RimTidy - todds texture encoder"
    ...
    def _do_check_for_workshop_updates(self) -> None:
        ...
        if updates_checked == "failed":
            dialogue.show_warning(
                ...
                text=self.tr("RimTidy was unable to query Steam WebAPI for update information!\n"),
    ...
    def _do_setup_steamcmd(self) -> None:
        if (
            ...
            dialogue.show_warning(
                title=self.tr("RimTidy - SteamCMD setup"),
        ...
        if local_mods_path and os.path.exists(local_mods_path):
            ...
            self.steamcmd_runner.setWindowTitle("RimTidy - SteamCMD setup")
    ...
    def _do_download_mods_with_steamcmd(self, publishedfileids: list[str]) -> None:
        ...
        if self.steamcmd_wrapper.steamcmd and os.path.exists(self.steamcmd_wrapper.steamcmd):
            ...
            self.steamcmd_runner.setWindowTitle("RimTidy - SteamCMD downloader")
            self.steamcmd_runner.show()
        ...
        answer = dialogue.show_dialogue_conditional(  # We import last so we can use gui + utils
            ...
            information=(
                self.tr(
                    ...
                    "If you just installed Git, please restart RimTidy for the PATH changes to take effect."
                )
            ),
        )
        if answer == QMessageBox.StandardButton.Yes:
    ...
    def _do_build_database_thread(self) -> None:
        ...
        if output_path:  # If output path was returned
            ...
            self.query_runner.setWindowTitle(
                f"RimTidy - DB Builder ({self.settings_controller.settings.db_builder_include})"
            )
    ...
    def _do_download_entire_workshop(self, action: str) -> None:
        ...
        self.query_runner = RunnerPanel()
        self.query_runner.closing_signal.connect(self.db_builder.terminate)
        self.query_runner.setWindowTitle("RimTidy - DB Builder PublishedFileIDs query")
        self.query_runner.progress_bar.show()
        self.query_runner.show()
        # Connect message signal
        self.db_builder.db_builder_message_output_signal.connect(self.query_runner.message)
            ...
            if "steamcmd" in action:
                ...
                answer = dialogue.show_dialogue_conditional(
                    ...
                    information=self.tr(
                        ...
                        + "This can take longer due to rate limits, but you can also re-use the script generated by RimTidy with "
                        + "a separate, authenticated instance of SteamCMD, if you do not want to anonymously download via RimTidy."
                    ),
                )
                if answer == QMessageBox.StandardButton.Yes:
                    for metadata_values in self.metadata_manager.internal_local_metadata.values():
    ...
    def _do_update_rules_database(self, instruction: list[Any]) -> None:
        ...
        answer = dialogue.show_dialogue_conditional(
            title=self.tr("RimTidy - DB Builder"),
