...
# Build RimTidy executable
build *ARGS='': submodules-init check
    uv run python distribute.py {{ARGS}}

# Build RimTidy executable with specific version (e.g., "1.2.3.4")
...
rpm-tarball VERSION='1.0.0':
    ...
    TARBALL="$HOME/rpmbuild/SOURCES/rimtidy-$FULL_VERSION.tar.gz"
...
build-rpm VERSION='1.0.0': check (rpm-tarball VERSION)
    ...
    rpmbuild -bb packaging/rpm/rimtidy.spec --define "version $FULL_VERSION"

    echo "RPM build complete!"
    RPM_FILE=$(find ~/rpmbuild/RPMS/x86_64/ -name "rimtidy-$FULL_VERSION-*.rpm" | head -n 1)
    if [ -n "$RPM_FILE" ]; then
